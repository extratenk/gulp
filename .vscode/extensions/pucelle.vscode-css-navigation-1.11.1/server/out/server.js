...
class CSSNaigationServer {
    ...
    formatLabelsToCompletionItems(labels, selector, document) {
        return labels.map(label => {
            ...
            item.textEdit = vscode_languageserver_1.TextEdit.replace(vscode_languageserver_1.Range.create(document.positionAt(selector.leftOffset), document.positionAt(selector.leftOffset + selector.value.length)), label);
            return item;
        });
    }
