...
class JSXScanner extends text_scanner_1.TextScanner {
    ...
    async scanSelector() {
        ...
        // 	 class="a b"
        // >
        let match = this.match(/<\w+\s*(.*?)>/g, /\b(?<type>id|class|className)\s*=\s*['"`](.*?)['"`]/g, /([\w-]+)/g);
        if (match) {
    ...
    async scanCSSModule(moduleName, moduleProperty, wordLeftOffset) {
        ...
        if (modulePath) {
            let fullPath = await file_1.resolveImportPath(path.dirname(vscode_uri_1.URI.parse(this.document.uri).fsPath), modulePath);
            if (fullPath) {
    ...
    parseDefaultImportedPath() {
        ...
        while (match = re.exec(this.text)) {
            let path = match[1];
            let extension = file_1.getPathExtension(path);
    ...
    async scanForImportPath() {
        ...
        if (modulePath) {
            return await file_1.resolveImportPath(path.dirname(vscode_uri_1.URI.parse(this.document.uri).fsPath), modulePath);
        }
        return null;
    }
}
